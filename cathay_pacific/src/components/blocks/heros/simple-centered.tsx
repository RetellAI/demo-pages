...
export default function SimpleCentered() {
  ...
  // White to light tan background gradient for hero area
  const gradient =
        'linear-gradient(130deg, #ffffff 0%, #f8f7ea 55%, #F7F6F1 100%)';
  // Button greens
  const lightGreen = '#22c55e'; // Light green theme
  const darkGreen = '#15803d'; // Darker green for text/contrast
        ...
        <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
          <div className="flex lg:flex-1">
            <a href="https://www.cathaypacific.com/cx/en_US.html" target="_blank" rel="noopener noreferrer" className="-m-1.5 p-1.5">
              ...
              <img
                alt="Cathay Pacific x Retell AI"
                src="/cathay-x-retell.png"
                className="h-auto w-auto" style={{ maxWidth: '70%', maxHeight: '70%' }}
              />
            </a>
          </div>
        ...
        <div className="mx-auto max-w-2xl w-full flex flex-col justify-center items-center">
          <div className="text-center flex flex-col items-center w-full">
            ...
            <div className="flex flex-col items-center mb-4">
              <h1 className="mt-2 text-4xl sm:text-5xl font-extrabold tracking-tight text-black">
                Try Our Live Demo
              </h1>
            </div>
            {/* Place this info on its own line above the input */}
            <div className="flex justify-center w-full">
              <p className="mt-5 text-lg font-medium sm:text-xl/8 whitespace-nowrap" style={{lineHeight: 1.25, color: '#265C62'}}>
                Receive a call from our Business Plus customer acquisition agent.
              </p>
            </div>
              ...
              <div className="relative flex w-full max-w-xs items-center gap-2 bg-white rounded-full px-3 py-3 border-2 border-white shadow-md">
                ...
                {showDropdown && (
                  <div
                    ref={dropdownRef}
                    className={
                      `absolute left-0 bottom-full mb-2 w-[260px] max-h-64 overflow-y-auto shadow-lg rounded-xl z-50 animate-in fade-in duration-100`
                    }
                    style={{ backgroundColor: '#F2F4F1', border: '1px solid #265C62' }}
                    tabIndex={-1}
                  >
                    {/* --- SEARCH BAR AT TOP --- */}
                    ...
                    >
                      {filteredCountryCodes.length ? filteredCountryCodes.map((c, idx) => {
                        ...
                        <div className="px-4 py-3 text-center opacity-60" style={{ color: '#265C62' }}>No matches</div>
                      )}
                    </div>
                  </div>
                )}
                ...
                <input
                  ...
                  className="flex-1 outline-none border-none bg-transparent text-green-800 placeholder:text-green-900 text-base placeholder:text-sm focus:outline-none focus:border-none focus:ring-0"
                  value={phone}
                  onChange={e => {
              ...
              <button
                ...
                className={`w-full max-w-xs rounded-full px-6 py-3.5 text-base font-semibold whitespace-nowrap shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:w-auto
                  ${callSent
                    ...
                    : 'hover:scale-105'}
                `}
                style={{background: callSent ? '#fff' : '#265C62', color: callSent ? '#265C62' : '#fff'}}
